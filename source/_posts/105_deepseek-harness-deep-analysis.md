...
在 AI 智能体的语境中，DeepSeek 这样定义两者的关系：**Agent = Model + Harness**

Model 负责思考、推理和内容生成。Harness 负责让模型的能力落地——调用外部工具、管理上下文状态、执行代码、维护对话记忆、协调多个子任务.

模型决定了系统的推理上限，Harness 决定了系统的执行下限。过去两年行业的注意力集中在模型能力的提升上——更大的参数量、更高的 benchmark 分数、更快的推理速度。但当开发者真正将模型部署到生产环境中时会发现，模型本身在整个系统中占比有限。绝大部分工程工作，比如工具集成、安全管控、状态管理、异常处理等，都属于 Harness 的范畴。
...
这一阶段的核心目标是让模型正确理解任务指令。典型做法包括调整措辞、添加 few-shot 示例、指定输出格式等。本质上是在解决"如何把需求表达清楚"的问题。甚至在一段时间，Prompt 工程师成了热门岗位。
...
行业随之意识到，让模型在正确的时机看到正确的信息，同样关键。这一阶段的核心是上下文工程，即通过 RAG 检索相关文档、管理记忆系统、压缩历史对话、将关键信息注入模型的上下文窗口。
...
DeepSeek Harness 的架构设计围绕一个核心原则：一切皆插件。
...
这些轨迹数据的用途不止于调试。它们可以作为 demonstration 数据用于后续的模型训练，也就是说，Agent 在实际使用中产生的执行记录，反过来为模型优化提供训练信号。这构成了一个数据飞轮：使用量越大，轨迹数据越多，模型能力越强。

---

## 五、与 Claude Code 的对比分析

| 维度 | DeepSeek Harness | Claude Code |
|------|-----------------|-------------|
| **开发者** | DeepSeek | Anthropic |
| **产品定位** | Agent Runtime / Agent 平台 | AI 编程助手 |
| **开源策略** | MIT 协议，完全开源 | 闭源商业产品 |
| **架构理念** | 插件化架构，组件可自由组合 | 一体化设计，深度绑定自家模型 |
| **模型绑定** | 不绑定特定模型，支持切换 | 深度绑定 Claude 系列模型，但也支持其他模型 |
| **运行模式** | 四种模式可选 | 单一模式 |
...
至于能否在生态建设和持续迭代中保持领先，还需要后续观察。从 2026 春节开始，时间好像按了加速键，我每个月都在见证 AI 的飞速发展，DeepSeek 对新产品的探索，我非常期待。

---

**参考资料：**
- [DeepSeek Harness 官网](https://www.deepseek.com/harness/?utm_source=chatgpt.com)
