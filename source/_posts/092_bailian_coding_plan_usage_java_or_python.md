---
title: Java / Python 应用调用阿里百炼 Coding Plan 服务
date: 2026-04-19
tags: Coding Plan
categories: AI
---
...
但实际上，Coding Plan 的额度完全可以在 Java/Python 应用中使用。本文将分享具体的调用方式。

---

## 二、Coding Plan Key 在 Java 中"失效"的原因
...
| dashscope-sdk-java 不支持 | LangChain4j/OpenAI SDK 可用 |

核心原理很简单：Coding Plan 的 endpoint 基于 OpenAI API 兼容协议，这是业界通用的 LLM API 标准。任何支持 OpenAI 协议的客户端都可以调用，不受编程工具限制。

实际使用中需要注意 endpoint 不要混用（`coding.dashscope.aliyuncs.com` vs `dashscope.aliyuncs.com`），模型名称按 Coding Plan 支持的列表选择，以及设置合理的超时时间。
