...
记原因的目的是让 Agent 能在新场景下做判断，而不是机械地执行规则。
...
用户说「周四冻结」，记忆里存的是具体的年月日，比如「2026-03-07」。原因是记忆跨会话的，如果存「周四」，下周再看这条记忆就不知道是哪个周四了。
...
这是四种里最简单的，但也是最实用的，本质上是一个「去哪里找信息」的索引。
...
这是最反直觉的一条。很多人觉得 Agent 应该记住「项目用了什么框架、目录怎么组织、哪个文件负责什么」，但 Claude Code 的设计原则是不要存这些。原因很直接：这些信息可以直接从代码里读出来，Agent 随时可以通过读代码和搜索来获取当前的项目结构。
...
背后的原则是：如果一个信息可以从当前项目状态推导出来，就不要存进记忆。记忆只存那些「看代码看不出来」的东西。

### 3.2 版本管理历史

谁改了什么、最近的提交记录，这些用版本管理工具查就行了。Git 是实时的、权威的，不需要记忆来存一份可能过时的副本。
...
Claude Code 记忆系统最值得学习的地方，不是它的存储格式或检索机制，而是那条分界线：记忆是代码的补集。这条原则把「什么该存」这个看似主观的问题，变成了一个有明确判断标准的设计决策。

从这个原则出发，四种该存的记忆有一个共同特点：它们都是代码里读不出来的东西。用户的偏好、纠正背后的原因、项目决策的动机、外部资源的位置，这些信息不存在于项目的任何文件里，只存在于人和上下文中。反过来，五种不该存的东西也有一个共同特点：代码、Git、配置文件已经给出了权威答案，再存一份只是制造噪声和过时风险。

做记忆系统最容易犯的错误是「宁可多存不少存」。但记忆文件的 token 成本是实打实的，每条记忆都会在每次对话中被加载。存十条低价值记忆带来的负担，远大于一条高价值记忆带来的帮助。少而精不是一句口号，是在 token 成本约束下的工程必然。
