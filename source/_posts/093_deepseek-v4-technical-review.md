...
双随机矩阵构成的空间是一个**流形（Manifold）**，即 Birkhoff Polytope。mHC 通过 Sinkhorn-Knopp 算法，把矩阵 B 投影到这个流形上：
...
- 用声明式语法描述 Kernel 逻辑，Z3 SMT Solver 做形式化分析（证明正确性），自动生成高性能 CUDA 代码。开发效率和运行效率兼得。

### 6.2 确定性训练

同一 token 无论在 batch 哪个位置，输出 bitwise 一致。用特殊设计避免了原子加法带来的不确定性，训练过程可复现，调试有据可查。这对大规模训练调试和定位问题至关重要。
...
- V4-Flash 用 13B 激活参数就能达到接近 GPT-5.2 的推理水平，在代码任务上开源首次追平闭源。闭源模型的护城河正在缩小。
...
DeepSeek V4 最值得关注的不是一两个刷榜指标，而是它系统性地解决了长上下文效率问题。CSA/HCA 混合注意力把 O(n²) 的复杂度打下来，mHC 让深层堆叠更稳定，Muon 优化器给万亿参数训练提供了新方案，FP4 量化训练从源头适配低精度推理。这些创新组合在一起的效果是：百万级上下文从理论上可行变成了经济上可行。

从行业格局看，V4-Flash 用 13B 激活参数达到了接近 GPT-5.2 的推理水平，Codeforces 排名追平闭源模型。开源模型在效率维度上第一次有了和闭源正面竞争的能力。

当然 V4 也有明显的局限：架构复杂度较高，部分稳定性技术的数学原理还没完全搞清，多模态尚未集成。这些是后续版本需要解决的问题。但就这次发布而言，DeepSeek 在架构创新上的投入密度，在国产大模型厂商中是最高的。
