...
这就好比查字典找一个词的完整释义，结果字典被人从中间撕开了，你只看到前半句"此药适用于"，后面的内容没了。你知道它适用，但你不知道适用于什么。
...
相邻 chunk 之间的重叠窗口，就像接力赛的交棒区：前一个选手和后一个选手有一段距离是共同持有的，这样交接的时候不会掉棒。
...
向量检索的打分是 0~1 之间的余弦相似度，BM25 的打分可能是十几甚至几十。两个维度的量纲完全不一样，合并需要解决量纲冲突问题。
...
这四步优化的共同特点是：每一步都不需要换更大的模型，不需要更贵的 GPU，纯粹靠工程手段在已有链路上挖掘提升空间。文档分块解决了信息碎片化问题，Query 校验守住了输入质量的下限，LambdaMART 重排让多路检索的分数有了科学合并方式，指标拆解让问题定位从猜测变成了精确诊断。

做完这四步之后，准确率从 60% 提升到 85% 并不算意外。真正让我觉得有价值的，是这个过程建立了一套可复用的优化方法论：先把链路摊开看全貌，再按优先级逐环拆解，每一步都有可量化的收益指标。RAG 系统从 Demo 到生产环境，靠的不是某个单点的突破，而是整条链路的工程化打磨。
